...
| T57 | Blur Faces & Plates | `/blur-face` | Local | Manually mark up to 12 rectangular regions or request optional YuNet face-region suggestions after an explicit, size-disclosed 227 KiB model download. Review and correct suggestions before downloading the local still-PNG result; missed faces remain visible unless marked. Plate detection is not included. The focused route suite passes 49/51 checks across Chromium, Firefox, and WebKit; two non-Chromium latency checks are skipped. A generated 6 MP PNG reached a one-region blur preview in a 229.5 ms median across five Chromium runs (217.8–336.8 ms); this synthetic timing does not establish broad-device speed or detection accuracy (§19.2) |
...
| T61 | Duplicate Finder | `/find-duplicates` | Local | Find exact SHA-256 duplicates and possible visual matches across up to 24 files (20 MiB/file, 80 MiB total, 24 MP/image). Near matches require human review; export a non-destructive CSV report. A controlled P4-21 supplement measured 34/60 same-source matches (including four exact-SHA detections) and 0/72 topic-matched false matches through Chromium's route-sized hash canvas; visually near-identical negatives and held-out thresholding remain open. Three matching thresholds now use a strict Zod schema and generated keyboard-accessible sliders with reset behavior; defaults preserve the prior route behavior. A warmed local 24-file selection completed with external requests blocked, and the 24-file route measured 991.8 ms in Chromium. Fresh offline startup, hard negatives, and held-out thresholding remain open; route criteria are traced in the STCC matrix. See [T61 benchmark](packages/engine/bench/escalation/p4-21-t61-duplicates.md) |
...
| T70 | Pixel-Art & Line-Art Upscale | /pixel-art-upscaler | Local | Deterministic palette-aware scaling. A narrow four-fixture engine benchmark found no invented RGB colors after a strict-majority guard; the opaque stepped fixture matched nearest-neighbour exactly, while transparent edges gained intermediate alpha. Its 16-case route suite passes in Chromium and installed Edge; Firefox and WebKit pass all 15 functional cases and skip only the latency measurement. It covers every typed error kind and remedy, keyboard use, offline reuse, and the 320 CSS-pixel layout without horizontal overflow in all four browsers. A fresh-page offline reload passed in Chromium; Firefox and WebKit skip that local worker/file decode case. Broad sprite preference and actual 400% browser zoom remain open, so broader sprite-quality limits remain open; route criteria are traced in the STCC matrix. See [T70 benchmark](packages/engine/bench/escalation/t70/REPORT.md) | None needed |
...
| T80 | Colour Match | `/color-match` | Local | Apply Reinhard transfer or per-channel histogram matching to two still PNGs (16 MiB / 6 MP each, 8 MP total) in a local worker, then compare and export the result. On three generated texture pairs, mean normalized channel-CDF distance was 0.065537 unchanged, 0.014468 with Reinhard, and 0.002413 with histogram matching. The method choice now uses a strict Zod schema and a localized keyboard-accessible generated segmented control with a reset affordance; the default remains Reinhard. Its focused route suite passes 27/27 across Chromium, Firefox, and WebKit, including keyboard use, Axe, local-only requests, exact preview/download bytes, typed input remedies, decode/worker failures, and worker-terminating cancellation. A warmed local match completed with external requests blocked; an 8 MP total PNG pair measured 1006.3 ms in Chromium. Spatial preservation, photo composites, and user preference remain unmeasured; invalid-PNG/canvas-unavailable/missing-image errors, fresh offline startup and broader quality limits remain open; route criteria are traced in the STCC matrix. See [T80 benchmark](packages/engine/bench/escalation/p4-21-t80-colour-match.md) | — |
| T81 | Adaptive Resize | /adaptive-resize | Local | Saliency-weighted continuous-warp retargeting (§25.4) with a protect mask. Target width, target height, and mask enablement now use a strict Zod schema and localized generated controls; zero remains accepted as an intermediate UI value so the route can return its typed invalid-dimensions remedy. In three generated scenes, feasible masked subjects retained their exact pixel area and bbox dimensions (1.000×, 0% aspect change), but shifted 8.08 px on average as background content was compressed. The unmasked path measured 19.29% mean bbox-aspect change versus 33.53% for resize. A warmed local resize completed with external requests blocked; a 320-pixel route measured 1420.6 ms in Chromium. Natural-photo quality, fresh offline startup, and broader quality and device limits remain open; route criteria are traced in the STCC matrix. See [T81 benchmark](packages/engine/bench/escalation/t81/README.md). It is not seam carving, which remains excluded (§25.3.2) | — |
...
| **Tier 1 — pixel-art scaler** | clean-room 3×3-neighbourhood implementation in packages/engine/src/cv/pixel-art.ts | Four generated fixtures measured at ×2/×3/×4; no RGB palette additions after the strict-majority fix. Representative sprite quality and broader quality and device limits remain open; route criteria are traced in the STCC matrix; GPL/LGPL reference implementations stay excluded | lazy |
...
| Face detection | Tier 1 Viola–Jones cascade plus optional, user-requested YuNet ONNX face-region suggestions through the pinned on-device ONNX Runtime Web | The exact YuNet file is MIT-registered at 232,589 bytes with a pinned SHA-256 and environment-overridable URL; its browser download is user initiated and never bundled or fetched by the app server. MediaPipe `.task` assets remain excluded. The synthetic CC0 corpus measures only functional and edge-case behavior; real-photo accuracy and broader quality and device limits remain open; route criteria are traced in the STCC matrix | Tier 1 manual path remains primary; optional Tier 2 suggestions |
...
| **Segment / click-to-select** (T27 assist) | **Tier 1:** colour range, watershed, flood fill, contours, and saliency. GrabCut is excluded. YuNet is cleared for face-region suggestions in T57 only; no semantic object detector or segmentation weights are available for T27 | Selecting an arbitrary named object without a usable colour or edge boundary; no comparative fixture is recorded yet | Semantic object identity | **Tier 1 hinted path; Tier 2 remains blocked for semantic object selection; no Tier 3 case admitted yet** |
...
| **OCR** (T62) | All 163 official recursive `tessdata_fast` entries are pinned and hash-registered: 123 language/variant binaries, one deprecated `frk` alias pointer to `deu_latf`, 37 script-model binaries, and two helpers (`osd` for orientation/script detection and `equ` for equations). Models load only after selection: local static data when present, otherwise the exact pinned jsDelivr file, except the 89,384,811-byte Latin script model which uses its exact pinned GitHub raw source because jsDelivr returns 403. Only `script/Cyrillic.traineddata` is bundled. The catalogue HEAD check passes for 161 other jsDelivr entries; a browser delivery smoke reached the pinned Latin fallback and returned OCR output; an independent streamed source check verified the registered byte length and SHA-256. The page recommends upright images and makes OSD optional. Eight generated-print PNG fixtures are persisted; accuracy measured 7/8 exact, Hindi CER 0.0588, mean CER 0.00735. One additional English paragraph from a registered Commons CC0 page measured one substitution (confidence 94, CER 0.00660). After online warm-up, same-page offline E2E repeated all eight language fixture recognitions with identical output and zero external requests; a fresh-page offline reload of the bundled Cyrillic path passed in Chromium and Firefox through the service-worker shell. A separate English CDN model proof passed a fresh-page external-network block in Chromium and Firefox after browser-side cache warm-up without another CDN request. Arabic and pseudo-locale selector labels were also browser-checked. Hausa is absent upstream. See [`OCR measurements`](packages/engine/bench/escalation/p4-21-t62-ocr.md). | The measured set contains eight simple generated print samples and one English CC0 page crop; it does not establish accuracy across the expanded catalogue, handwriting, artistic type, tables, photos, or skewed captures. WebKit skips the browser-wide offline reload because its local File/Blob flow returns an internal error; CDN cache persistence after browser eviction, private browsing, or site-data clearing remains unverified. | Better recognition on those unmeasured classes | **OCR selects one model at a time from the pinned catalogue, fetching missing files directly from their registered public sources; accuracy has eight generated-language fixtures plus one English CC0 sample, warmed-cache same-page offline reuse passed, bundled-Cyrillic fresh-page offline reuse passed in Chromium/Firefox, and CDN English fresh-page network-blocked reuse passed in Chromium/Firefox; broader accuracy and cache persistence after eviction/private browsing remain open** |
| **Face-region suggestions** (T57) | **Tier 1:** manual region marking and cleared Viola–Jones cascade. **Tier 2:** optional user-started YuNet ONNX inference on-device, using OpenCV Zoo's MIT-licensed `face_detection_yunet_2023mar.onnx` pinned by exact revision, 232,589-byte size, and SHA-256. The browser downloads and verifies the model only after the user requests suggestions; an environment-configurable host may serve only identical registered bytes. The generated fixture suite contains three deterministic project-generated CC0 fixtures with five exact boxes and yielded 5 TP, 0 FP, 0 FN at IoU ≥ 0.50 (per-box IoU 0.7575–0.8830). See [`T57 measurements`](packages/engine/bench/escalation/p4-21-t57-face-blur.md) and [`model delivery`](docs/t57-yunet-model-hosting.md). | This generated fixture suite checks front-facing shapes and functional edge cases only. It does not establish real-photo accuracy, pose/lighting/demographic coverage, plate detection, or safe redaction; misses remain visible unless users add regions. | More initial face-region suggestions on supported images, subject to review | **Manual Tier 1 remains available; optional Tier 2 suggestions are experimental until real-photo validation. MediaPipe `.task` assets remain excluded.** |
...
- Service worker: the shipped hand-rolled `apps/web/static/sw.js` is registered from the root layout.
  - App shell and route HTML: network-first, with the last successful route response as an offline fallback.
  - Hashed JS/CSS/fonts, same-origin WASM, model-runtime assets, and the Tesseract runtime: cache-first,
    immutable. Traineddata files are deliberately not service-worker cached; Tesseract's own browser
    cache/IndexedDB owns selected model data, and only `script/Cyrillic.traineddata` is bundled.
  - ONNX model bytes remain user-started and are cached by the T32/T57 consented loaders, not by the
    service worker.
  - Anything cross-origin: **never cached, never intercepted.** CDN model requests bypass the service
    worker entirely — a service worker that touched a request carrying a credential would be an
    unnecessary trust surface.
...
After an online warm-up, fresh-page offline E2E passes for the T32 Tier 1 route in Chromium and for the
bundled T62 Cyrillic model in Chromium and Firefox. A separate English CDN model proof passes a fresh-page
external-network block in Chromium and Firefox after browser-side cache warm-up without another CDN request.
WebKit skips browser-wide offline reload because its local File/Blob flow returns an internal error. CDN
model persistence after eviction, private browsing, or site-data clearing is not guaranteed.
...
counsel; T69 composite comparisons remain open. T80 has a generated global-distribution benchmark, while photo preference and broader quality and device limits remain open; route criteria are traced in the STCC matrix.
...
(T68) remains excluded pending exact asset review; T57 now offers optional, integrity-checked YuNet face suggestions, while MediaPipe `.task` assets remain excluded. Tesseract's full
...
| Face detection | Viola–Jones cascade (Tier 1); optional OpenCV Zoo YuNet ONNX suggestions (Tier 2, MIT asset) | Viola & Jones 2001; YuNet registered in `docs/model-assets.json` | — |
