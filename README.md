...
bundled T62 Cyrillic model in Chromium and Firefox. WebKit skips the latter browser-wide offline reload
because its local File/Blob flow returns an internal error. CDN-delivered OCR models and Tier 2 ONNX
models still require their browser-side cache/IndexedDB entries and are not promised after eviction,
private browsing, or site-data clearing.
