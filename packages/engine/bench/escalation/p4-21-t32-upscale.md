...
**Status:** Tier 1, Real-ESRGAN ×2/×4, and an evaluation-only Swin2SR ×4 candidate have measured results on same-fixture synthetic tasks. Real-ESRGAN trailed Tier 1 on the clean 2× interpolation set and all controlled ×4 categories. Swin2SR improved aggregate PSNR and SSIM across the 16 controlled ×4 cases, with mixed per-class PSNR and a slow browser WASM smoke; it is not selected for product use. These CC0-derived fixtures are not real degraded camera captures; broader restoration quality and product model delivery remain open.
...
The clean x2 comparison and Real-ESRGAN x4 comparison show no Tier 2 quality advantage on their tested pairs. Swin2SR has a modest aggregate synthetic advantage but loses clean-class PSNR, runs slowly in the measured CPU/WASM smokes, and still lacks established per-file licensing/training provenance. Keep Tier 1 as default and keep Swin2SR evaluation-only; do not present either model as a product quality improvement yet. The engine adapter accepts caller-supplied weights and consent only; the product route, size disclosure, consent UI, download, and persistent-cache flow are not implemented. Real degraded-photo fixtures, representative image sizes, browser steady-state latency/memory, and product model hosting remain open. Do not generalize these synthetic results to every restoration task.
