...
The input corpus contains four different 128×128 central crops and deliberately generated variants. It does not include difficult visual comparisons, alignment/registration, alpha, animation, large images, or human judgments. The engine `approximateSSIM` name is retained as implemented, but its current output is `1 - mean absolute Rec.601 luminance error / 255`, not structural SSIM. T60's product route and route-level STCC remain unverified. Broader fixtures and interaction evidence are still needed before claiming general comparison quality.
