...
export function makeCollage(
  image: RasterImage,
  options: Partial<CollageOptions> & { images: readonly RasterImage[] },
): RasterImage {
  ...
  const cols =
    options.columns ??
    (mode === 'horizontal' ? images.length : Math.ceil(Math.sqrt(images.length)));
  const rows =
    options.rows ?? (mode === 'vertical' ? images.length : Math.ceil(images.length / cols));
  ...
  const outputW =
    mode === 'horizontal'
      ? maxW * images.length + gap * (images.length - 1)
      : maxW * cols + gap * (cols - 1);
  const outputH =
    mode === 'vertical'
      ? maxH * images.length + gap * (images.length - 1)
      : maxH * rows + gap * (rows - 1);
  ...
  return {
    ...
    height: outputH,
    frames: [
      { ...image.frames[0]!, data: output, width: outputW, height: outputH },
