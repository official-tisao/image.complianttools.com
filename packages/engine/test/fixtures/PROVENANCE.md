...
truncated input, 1×1 and 30,000×1 QOI images, a QOI header declaring an unsafe pixel count, and an SVG
with an external HTTPS image reference. Each malformed input must return normally with an error and must
never allocate from hostile declared dimensions or cause a local SVG workflow to request network resources.
