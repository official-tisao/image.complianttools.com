...
The focused route E2E currently covers primary HTTP 403 followed by an unavailable fallback, with Tier 1 still usable. It does not yet cover a successful product-route download and inference, visible byte-progress behavior, cancellation, hash-mismatch rejection, or warm-cache reuse after going offline. Those checks and the deployment's host-longevity decision remain open under P4-20.
