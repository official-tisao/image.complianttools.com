...
| Real-ESRGAN weights                | The exact official x2 `v0.2.1` and x4 `v0.1.0` source checkpoints are registered in `docs/model-assets.json`. Per owner direction, the upstream BSD-3 label is accepted for these official `.pth` files with asset-level terms risk; this is not a separate upstream statement about the files. Both ONNX conversions and CPU parity checks passed, and x2 passed a browser WASM smoke. Community fine-tunes remain excluded. x4 browser validation, representative performance/quality measurement, and product model hosting/UI remain open, independent of this source-weight approval. |
...
These rows clear the source checkpoints as assets only. The converted ONNX files are separate runtime
inputs: both conversions passed CPU parity and x2 passed a browser WASM smoke, but x4 browser
validation and product model hosting/UI remain open. Those runtime and product gates do not alter the
source-checkpoint asset decision.
...
- No `.task` file, MediaPipe weight, or other excluded model asset is referenced or loaded by the registered entry.
