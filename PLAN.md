...
- [ ] `packages/engine/bench/escalation/` has side-by-side local-vs-reference outputs and a measured shortfall for every applicable register capability
...
- [ ] Fill in README §13.1.3 **from measured results, not assumptions** for every applicable register entry
- [ ] **Delete the Tier 3 row for any capability whose Tier 1/2 path turned out good enough**
- [ ] Log every deletion in §16 — this is a success, not a deviation
- **Spec:** README §13.1.3 · **Done when:** each register row has status-appropriate evidence in `bench/escalation/`: an active Tier 1/2 escalation has a measured comparison and shortfall; a blocked row names the concrete implementation or clearance blocker; a no-model-gap or AI-only row records its rationale; and a measured row states the tested scope and limits. No row is upgraded or removed without evidence.
