...
6. **If a task turns out to be wrong, blocked, or unnecessary**, do not silently skip it. Mark it
   `[~]` with a one-line reason and add a line to §16 Change Log. An unexplained skip is a defect.
...
| `[~]` | Deferred or dropped — **reason required inline**, and a §16 Change Log entry |
...
| --- | --- |
| New feature, tool, or option | Add a task in the correct phase **and** a row in Appendix A/B/C. Add a §16 Change Log entry |
| Changed requirement on unbuilt work | Amend the task text; note in §16 |
| Changed requirement on **already-checked** work | **Uncheck the box**, add a `-R` revision task (e.g. `P2-07-R1`), note in §16. A completed task whose spec moved is no longer complete |
| Removed feature | Mark the task `[~]`, note why, note in §16 |
...
| Clarification with no behavioural change | §16 entry only, no task |
...
- [x] `onnxruntime-web@1.30.0` is pinned and MIT-verified; real T32 x2 and x4 ONNX models ran through Chromium's WASM execution provider with same-origin runtime/model requests and no remote requests
- [/] Lazy engine API, WebGPU/WASM selection, Tier 1 default/fallback, and an explicit T32 download flow are wired. The selected Hugging Face x2/x4 exports are pinned to full commit `d14119a40dfeef208e4e724dfaceb2640d2df95b`, size/hash registered, CPU-parity verified against the approved `.pth` source, and browser-WASM smoke tested. Chromium fetched both full model files cross-origin from a local test origin with CORS enabled and matching byte counts/SHA-256; this verifies the current URLs, not future uptime. Docker runtime env config supplies pinned primary URLs and optional per-model fallbacks; it never fetches model bytes. Focused Chromium route E2E verifies registered x2 delivery and cache lifecycle plus x2/x4 inference. A fresh-page Chromium test now reloads the same-origin shell/worker/core offline after service-worker warm-up and runs Tier 1; cross-origin Tier 2 model availability after a fresh offline start, production deployment availability, and full STCC remain to be verified.
- **Spec:** README §7.2, §13.1.2 · **Done when:** a consented, size-disclosed model downloads once, caches, and runs offline thereafter in a browser
...
#### P4-16 · Upscale model
- [x] Official Real-ESRGAN x2/x4 release `.pth` checkpoints are recorded by exact release, size, and SHA-256 in `docs/model-assets.json`; the project owner directs use of the upstream BSD-3 label for these files and accepts the asset-level terms risk; community fine-tunes remain excluded
- [x] Public Hugging Face ONNX x2/x4 exports are pinned to an immutable full commit and registered by exact URL, size, SHA-256, and publisher-declared BSD-3 model-card evidence; both passed CPU parity against their source checkpoints and Chromium WASM smoke
- [x] Initial same-fixture x2 comparison is recorded: Real-ESRGAN scores 26.6370 dB / 0.765509 mean SSIM on four synthetic 2× pairs, below DCCI (29.6861 / 0.827896) and Lanczos3 (30.7267 / 0.858497); CPU latency is measured and correctly kept separate from Node timings
- [x] A 16-pair controlled synthetic x4 comparison is recorded; the pinned Real-ESRGAN export trails Tier 1 in PSNR and SSIM on all pairs, with CPU and browser-WASM timings recorded
- [/] The T32 route now downloads only on user action, reports progress, supports cancellation and optional per-model URL fallback, validates size/SHA-256, caches in IndexedDB, probes browser inference, and keeps Tier 1 available. Focused Chromium route E2E passes on the registered x2 model: a 3×2 input runs successfully and produces the requested 6×4 output after even-dimension edge padding/crop; verified model bytes are reused from IndexedDB after reload with the model host unavailable; a same-size SHA mismatch is rejected without fallback; and a cancelled partial transfer is not cached. A separate route E2E runs the registered x4 model on the same odd-size input and verifies the requested 12×8 output. A fresh-page Chromium route test warms the app shell/worker/core, reloads offline, and runs Tier 1; this does not establish that the cross-origin Tier 2 model survives a fresh offline start. Production-origin longevity, representative degraded-photo quality, and full route STCC remain open; do not infer a general restoration benefit from the synthetic comparisons
- **Spec:** README §25.5, §13.1.3 · **Done when:** exact checkpoints execute in the browser and a measured comparison is recorded in `bench/escalation/`

#### P4-17 · Face and plate detection
- [/] Cleared Viola–Jones Tier 1 plus optional user-started YuNet ONNX suggestions; MediaPipe `.task` assets remain excluded
- [/] Review-before-apply is implemented for suggestions; plate detection and batch operation remain open
- **Spec:** README §4.6 · **Done when:** T57 detects and blurs faces on a test set, with a review step

#### P4-18 · OCR
- [x] All 163 recursive `.traineddata` tree entries in official `tessdata_fast` commit `87416418657359cb625c412a48b6e1d6d41c29bd` are hash-registered: 123 language/variant model binaries, one deprecated `frk` alias pointer, 37 script model binaries, and the `equ` / `osd` helper binaries. Only the selected model is loaded: Cyrillic is the only bundled static model; missing models are fetched directly by the browser from the exact pinned jsDelivr, except the 89,384,811-byte Latin script model uses its exact pinned raw GitHub source after jsDelivr's 403. The Latin fallback passes a browser OCR delivery smoke and full size/SHA-256 verification, and the option displays its 85.2 MiB size before use. The CDN-blocked `script/Cyrillic.traineddata` is the only bundled OCR data file; 161 other jsDelivr paths pass metadata-only availability checks. Test/benchmark setup downloads only requested models into the ignored static cache and verifies size/hash. Clean production builds prune that cache. Hausa is absent from the upstream snapshot
- [x] `tesseract.js@7.0.0` recognizes persisted, hash-verified 1200×160 generated PNG fixtures for the original eight selected languages from same-origin worker/core and hash-verified local test-cache data; 7/8 fixtures were exact and mean CER was 0.00735. The original measured RGBA hashes are preserved in the fixture manifest. This measurement does not establish accuracy for the other catalogue entries
- [x] After online warm-up, the same-page browser test repeats all eight registered measurement-language fixtures offline with identical text and no external requests; this exceeds the five-language cached-offline threshold in the `Done when` criterion
- [x] A fresh-page offline reload of the bundled Cyrillic path passes in Chromium and Firefox after service-worker warm-up; WebKit skips this browser-wide offline reload because of its internal local File/Blob limitation. A separate English CDN model proof passes a fresh-page external-network block in Chromium and Firefox after browser-side cache warm-up without a second CDN request. CDN persistence after eviction/private browsing remains unverified.
- **Spec:** README §25.3.4 · **Done when:** T62 extracts text in at least five registered languages offline after the documented first-use/cache flow
...
- **STCC evidence update (2026-09-22):** The route-level matrix in [`docs/p4-20-stcc-matrix.md`](docs/p4-20-stcc-matrix.md) supersedes the earlier progress narrative below. It traces all twelve criteria for the eleven routes using the route suites, default/offline/error checks, measured route budgets, and documented model/cache qualifications. Real-photo quality, broad-device performance, host durability, and model-cache eviction remain register limitations rather than unverified STCC claims.
- [/] Engine primitives or adapters and dedicated product routes cover T27 Smart Crop, T32 Upscale, T57 manual region blur with optional YuNet suggestions, T60 Compare, T61 Duplicates, T62 OCR, T63 manual alt-text review, T70 Pixel-Art Upscale, T79 Procedural Generator, T80 Colour Match, and T81 Adaptive Resize; their route-level STCC is traced in the evidence matrix; broader quality, host, and device limits remain qualified. T27's ratio and placement options now use a Zod schema and the shared generated-control renderer, default to original-ratio/center so the full image is preserved, and have route coverage for keyboard selection plus all nine route error kinds. Its 12-case CC0 placement benchmark measures mean subjective target-box retention of 89.82% center, 76.70% thirds, and 71.97% approximate saliency. A generated 12 MP square-crop route preview measured 1427.8 ms median (five local Chromium runs, range 1416.8–1467.1 ms); the 3 s budget is asserted in route E2E. These synthetic timings and subjective boxes do not establish broad-device latency, visual quality, or preference. A fresh-page offline reload passed in Chromium; Firefox and WebKit skip this local worker/file decode case while same-page offline coverage remains active. T32's DCCI/NEDI `/upscale` route runs in a local worker, accepts bounded still PNGs, and passed a focused fresh-preview Chromium suite 9/9 plus a separate active-source-replacement regression 1/1. Tier 2 has a user-started, size-disclosed download with progress, cancellation, size/SHA-256 verification, IndexedDB reuse, an inference probe, and Tier 1 recovery. Focused Chromium E2E verifies actual registered x2 and x4 inference on 3×2 PNGs (6×4 and 12×8 outputs after even-dimension edge padding/crop); the x2 route additionally verifies host-unavailable IndexedDB reuse after reload, hash-mismatch rejection, and cancellation without caching partial bytes. This does not verify a fully offline app-shell/worker start, durable production hosting, representative performance or real-camera quality; broader quality, host, or device limits remain open; route criteria are traced in the matrix. The ~64 MiB model files stay outside Git and the Docker image. T32 output-byte limits were rejected in E2E across Chromium, Firefox, and WebKit. T60's checked-in route E2E passes 27/27 checks across Chromium, Firefox, and WebKit: registered CC0/JPEG comparison, side/onion modes, mismatch score suppression, Axe, no off-origin requests, typed unsupported/oversize/decode/worker errors, cancellation with worker termination, and keyboard navigation/activation for modes and split. Image-too-large and canvas-unavailable branches, hard comparisons, representative quality, broad-device latency, and broader quality or host limits remain open; route criteria are traced in the matrix. A Chromium production-preview run measured a 12 MP pair at 339.3 ms against the 5 s route budget. Its ≤256-pixel score is only a proxy. T70's 16-case route suite passed Chromium and installed Edge; Firefox and WebKit passed all 15 functional cases and skipped only the Chromium/Edge-only latency measurement. Its 320 CSS-pixel layout passed without horizontal overflow in all four browsers; a fresh-page offline reload passed in Chromium, while Firefox and WebKit skip that local worker/file decode case; actual 400% browser zoom remains unverified. T57's manual region and opt-in suggestion paths are covered by route E2E; actual YuNet browser inference matched all five boxes in the selected synthetic CC0 corpus. The model is requested only after user action and byte/hash checked; manual marking remains available. This evidence establishes generated-case behavior only, not real-photo accuracy, plate detection, or safe automatic redaction. A generated 6 MP solid-color PNG reached a one-region blur preview in a 229.5 ms median over five local Chromium runs (217.8–336.8 ms) against the ≤3 s gate. T57 broader real-photo and device limits remain open; route criteria are traced in the matrix. T63 now has an offline manual alt-text writing route with local image preview, an editable 125-character draft, decorative marking, human-review reminders, and an escaped/copyable HTML attribute preview. Its focused fresh-preview route suite passed 22/24 checks across Chromium, Firefox, and WebKit; the two skipped checks are Chromium-only clipboard reads. Coverage includes localized prerender/SEO, keyboard use, Axe, no external requests, and image and draft error remedies. A generated 6 MP selection-to-preview measured 175.7 ms in Chromium, 211.6 ms in Firefox, and 656.9 ms in WebKit in one local run. These synthetic timings do not establish broad-device latency or accessibility outcomes; T63 is not automated accessibility analysis and broader quality limits remain open; route criteria are traced in the matrix. T61 reuses shared analysis primitives and now exposes its three bounded matching thresholds through a strict Zod schema and generated resettable controls; defaults preserve the prior route behavior. A warmed local 24-file selection passed with external requests blocked and measured 991.8 ms in Chromium. T80 and T81 also passed warmed local operations with external requests blocked, measuring 1006.3 ms for an 8 MP total pair and 1420.6 ms for a 320-pixel resize. T81 now exposes target width, target height, and mask enablement through a strict Zod schema and generated controls while preserving its typed invalid-dimensions path. T27/T81 local fixture readiness waits now allow 15 seconds for the browser decode/control transition; the focused three-browser rerun passed 51/57 with 6 documented skips. The eleven route STCC rows are checked in Appendix A; broader quality and host limits remain qualified in the matrix.
- [/] T32 delivery direction: Tier 1 loads immediately; Tier 2 is an explicit, size-disclosed browser download with progress, byte-count/SHA-256 verification, IndexedDB reuse, and post-download runtime/inference checks. Tier 1 remains available on unsupported devices or any delivery failure. The primary model URLs use the selected public Hugging Face repository pinned to full commit `d14119a40dfeef208e4e724dfaceb2640d2df95b`; per-model Docker runtime overrides and optional fallbacks can replace an inaccessible origin. Browser CORS, full byte transfer, and hash checks passed for the checked revision, but a third-party public repository has no persistence guarantee. Docker supplies URLs only and never downloads model bytes. See [`docs/t32-model-hosting.md`](docs/t32-model-hosting.md).
- [x] T27 Smart Crop · T32 Upscale (Tier 1+2) · T70 Pixel-Art Upscale · T79 Procedural Generator · T80 Colour Match · T81 Adaptive Resize · T57 Manual Face-Region Blur · T60 Compare · T61 Duplicates · T62 OCR · T63 Alt Text Review — STCC; route evidence is traced in [`docs/p4-20-stcc-matrix.md`](docs/p4-20-stcc-matrix.md), with Tier 2/CDN cache qualifications and browser-specific offline limitations stated explicitly
- **Spec:** README §4 · **Done when:** Appendix A rows checked

- [/] T80's focused production-preview route suite passes 27/27 across Chromium, Firefox, and WebKit, covering localized rendering, preview/export byte identity, keyboard controls, Axe, no off-origin requests, existing input remedies, simulated decode and worker-construction failures, and cancellation with worker termination. This adds route error-path evidence but no new quality or latency measurement. T80 still lacks invalid-PNG/canvas-unavailable/missing-image route assertions, a route-scale latency gate, offline operation evidence, and representative photo/preference data; its broader quality, host, or device limits remain open; route criteria are traced in the matrix.
- [/] T61's focused production-preview route suite passes 27/27 across Chromium, Firefox, and WebKit, including every declared route error kind and remedy: too-many, too-large (per-file and batch), unsupported MIME, decode, pixel limit, hashing, and cancellation. This leaves complete SEO/i18n evidence, route-scale latency/offline proof, visually near-identical duplicate negatives, held-out thresholding, and full STCC open.
- [/] T57 keeps manual region marking and adds optional user-started YuNet suggestions. The exact OpenCV Zoo model is MIT-registered and hash/size verified in the browser; the container only supplies an environment-configurable URL and never stores or downloads model bytes. The selected CC0 corpus contains three deterministic synthetic faces scenes with five exact boxes. Chromium matched all five at IoU ≥ 0.50 (5 TP, 0 FP, 0 FN); this checks functional and edge cases only, not real-photo accuracy, plate detection, or safe automatic redaction. See `packages/engine/bench/escalation/p4-21-t57-face-blur.md`.

#### P4-21 · Escalation benchmark corpus
- [/] T44 has a measured production-API comparison: four registered CC0 crops × three deterministic noise classes, with median/bilateral fixture and output hashes, PSNR/SSIM, and CPU latency. Bilateral improved both metrics on Gaussian σ12/σ24; median improved 2% impulse noise. This does not cover raw sensor noise, broad photo quality, or the absent `/denoise` route STCC; wavelet/BayesShrink is not implemented and NLM remains excluded.
- [/] Five individual CC0 image sources are item-registered; T27 and T32 use the original four, and T62 adds a separately registered Commons OCR page with one English paragraph crop measured at confidence 94 and CER 0.00660 (one substitution). T27 compares the route's center, thirds, and approximate-saliency placements on 12 reproducible square-crop cases; mean manually annotated target-box retention is 89.82%, 76.70%, and 71.97% respectively. The four boxes remain subjective annotations and do not establish visual quality or preference. Its generated 12 MP PNG route preview took a median 1427.8 ms across five local Chromium runs (1416.8–1467.1 ms); the ≤3 s click-to-preview gate is checked in E2E and the synthetic image does not establish broad-device latency. T32 Tier 1 and Real-ESRGAN x2 have same-fixture clean-interpolation measurements; Real-ESRGAN x4 has 16 measured outputs on four CC0-derived clean/blur/JPEG/grain synthetic cases and trails Tier 1 on both metrics in every case. The selected public ONNX exports are pinned, size/hash registered, CPU-parity verified, browser-WASM smoke tested, and fetched successfully with browser CORS from their current host; real degraded camera captures and deployed-host persistence remain unverified. A separate exploratory Swin2SR q4f16 x4 record uses the same 16 inputs, but the asset is not cleared and its results are excluded from product selection; it remains review-only pending asset and real-photo evidence. T60 has 26 controlled comparisons over four registered CC0 sources, with exact-copy and JPEG-quality ordering checks passing 4/4; T61 has 552 initial fixed-threshold pairs plus a separate 12-image, 132-pair Chromium supplement using the route's 8x8 preprocessing and current thresholds (34 TP, 0 FP, 26 FN, 72 TN; P=1.00, R=0.57). Its two item-registered CC0 photos are topic-matched rather than visually near-identical negatives; held-out thresholds, broader hard negatives, and route STCC remain open. T66 retains the initial five-method comparison on three generated known-target textures (21 hashed artifacts) and adds eight photo-derived synthetic-occlusion cases from the four registered CC0 sources. Five methods have 40 hashed case outputs; Telea leads masked-region mean PSNR (25.3583 dB) and SSIM (0.610996), while Navier–Stokes is fastest by median (0.320 ms). These resized 32×32 known-pixel occlusions do not measure genuine object removal, large fills, or structural reconstruction. T70 has four generated palette fixtures at three integer scales; the strict-majority fix removes invented RGB colors, with transparent-edge alpha tradeoffs measured. T80 has three generated texture comparisons: normalized global RGB distribution distance falls from 0.065537 unchanged to 0.014468 Reinhard and 0.002413 histogram mapping, with no photo-preference claim. T81 has three generated retarget scenes and four subjects; feasible protect masks preserve exact measured subject dimensions with 1.000× area and 0% bbox-aspect change, while subjects shift 8.08 px on average; the pre-fix compression results are preserved. T62 has eight same-origin browser measurements on persisted, hash-verified PNG inputs, all 163 recursive official data entries registered, a pinned raw-source fallback for the oversized Latin script model, metadata-only availability evidence for the remaining CDN paths, and same-page offline repeat evidence for all eight measured models. Synthetic measurements do not establish broad product quality. The added route evidence records fresh-page Chromium offline checks for T27/T70 and warmed local external-network-blocked checks plus measured latencies for T61/T80/T81; broader quality, cache, and host limitations remain open as named in the corpus index; route criteria are traced in the matrix.
- [/] T79's local outcome is measured with 15 reproducible generated cases across five production engine operations and three dimensions. Fixed-seed RGBA SHA-256 matched across seven runs per case; this recorded run's pooled Node operation medians span 1.3975–42.3808 ms. The generated outputs are not visual-quality references; browser timing and user preference remain open, and this deterministic feature scope provides no demonstrated AI gap. See [`packages/engine/bench/escalation/p4-21-t79-procedural.md`](packages/engine/bench/escalation/p4-21-t79-procedural.md).
- [/] T57 uses the selected generated fixture suite: three deterministic CC0 PNG scenes, five exact face boxes, and recorded PNG/pixel hashes. The pinned MIT YuNet model achieved 5 TP, 0 FP, and 0 FN at IoU ≥ 0.50 in a Chromium production-preview smoke; per-box IoU was 0.7575–0.8830. The report and browser output retain the exact model, runner, fixture, prediction, and timing data. This is a synthetic functional/edge-case measurement and establishes no real-photo detection accuracy, demographic or pose coverage, plate detection, or safe redaction. See [`packages/engine/bench/escalation/p4-21-t57-face-blur.md`](packages/engine/bench/escalation/p4-21-t57-face-blur.md).
- [x] `packages/engine/bench/escalation/` has side-by-side local-vs-reference outputs where a cleared reference exists, and a measured local shortfall or concrete blocker for every other applicable register capability; T27 segment, T67 expand, T68 background removal, and T69 background replace now have reproducible generated reports
- [x] **A written statement per capability of exactly where the local path falls short**; AI-only and unavailable rows are explicitly marked `NOT APPLICABLE`, `BLOCKED`, or `PARTIAL` in the corpus index
- **Spec:** README §26 Phase 4 exit · **Done when:** every capability in the register has a measured entry

#### P4-22 · Register reconciliation — complete
- [/] T44's AI register row now describes only implemented median/bilateral filters and includes measured noise-class tradeoffs. A learning-based denoiser is not justified by this controlled synthetic corpus, which does not establish low-light or sensor-noise failure.
- [/] README §13.1.3 assigns Real-ESRGAN to T32 (T70 remains pixel-art-only), records the owner-directed risk decision for the exact `.pth` checkpoints without presenting it as an upstream per-file license statement, and states that the tested synthetic x2/x4 results do not show a Tier 1 quality advantage. Both exact Hugging Face exports are pinned and registered; `onnxruntime-web`, license evidence, CPU parity, full-browser CORS/hash delivery, and x2/x4 Chromium WASM smoke results are recorded. The opt-in download/progress/cancel/integrity/cache/runtime-probe flow is implemented, with primary/fallback URLs configurable at Docker runtime and no model bytes fetched by the app container. Registered x2 and x4 product-route inference pass in Chromium; the x2 route also verifies model-cache reuse with the host unavailable, hash rejection, and partial-transfer cancellation. A fully offline app-shell/worker start, production-origin durability, and real degraded-photo evidence remain open; route criteria are traced in the matrix. Swin2SR remains an unapproved exploratory record, excluded from product selection pending exact asset clearance. T62 records the 163-entry catalogue, pinned CDN/raw-source delivery, browser-side cache, and only Cyrillic as a bundled model; test cache pruning and Docker exclusion remain required safeguards. T27 now has a narrow four-image crop-placement report. T66 retains its eight-case CC0-derived proxy limitations; P4-21 includes T60/T61, T70, T80, and T81 measurements. Remaining entries retain evidence-based partial, blocked, or no-model-gap status, with broader quality and host limitations open; route criteria are traced in the matrix.
- [/] T57's register entry now records the MIT-cleared, pinned YuNet model and the optional browser-only suggestion path. The `T57_YUNET_MODEL_URL` runtime setting changes delivery host without changing the allowed bytes; Docker does not fetch or package the model. The selected synthetic CC0 corpus measures only the registered generated cases, and real-photo accuracy remains open.
- [x] Fill in README §13.1.3 **from measured results, not assumptions** for every applicable register entry; the reconciled audit is [`docs/p4-22-register-audit.md`](docs/p4-22-register-audit.md)
- [x] **Delete the Tier 3 row for any capability whose Tier 1/2 path turned out good enough** — audited as not applicable because the register has capability rows, not shipped Tier 3 provider rows; no deletion was warranted
- [x] Log every deletion in §16 — no register rows were deleted in this reconciliation, and the audit records that decision
- **Spec:** README §13.1.3 · **Done when:** each register row has status-appropriate evidence in `bench/escalation/`: an active Tier 1/2 escalation has a measured comparison and shortfall; a blocked row names the concrete implementation or clearance blocker; a no-model-gap or AI-only row records its rationale; and a measured row states the tested scope and limits. No row is upgraded or removed without evidence. The completed reconciliation is recorded in [`docs/p4-22-register-audit.md`](docs/p4-22-register-audit.md).
...
| 2026-09-22 | §4, §26 | Added route-level P4-20 STCC traceability for all eleven target tools: default no-op assertions, fresh-page/local offline checks, additional typed error branches, keyboard paths, and the P4-21 register gate. The matrix records Tier 2/CDN cache qualifications and the known WebKit local File/Blob limitation. A local Chromium rerun passed 150/154 checks; the four model-dependent T32 checks were blocked by the current external-host timeout. | Closed the route-level STCC evidence item without converting synthetic or cache-dependent results into real-photo or production-host guarantees |

Every README change gets a row here, per §0.3. Newest first.

| Date | README § | Change | PLAN action |
| --- | --- | --- | --- |
| 2026-09-22 | §13.1.3, §26 | Added measured P4-21 reports for T27 segment assist and the T67/T68/T69 cutout/fill paths, plus a register gate that checks all 21 corpus rows and their linked evidence. The selected T57 corpus remains the deterministic in-repository CC0 synthetic face suite for functional and edge-case tests only. Reconciled the current audit, STCC matrix, and P4-22 register audit; route-level criteria are traceable while real-photo quality, broad-device behavior, and model-host durability remain qualified. | Recorded the measured shortfalls and register reconciliation without upgrading AI-only or model-dependent claims | 
| 2026-09-22 | §26 | The full CI-mode three-browser matrix completed with 1,101 passed and 49 documented skips. Two Chromium local-fixture readiness retries in T27/T81 were stabilized with 15-second decode/control waits; the focused three-browser rerun passed 51/57 with 6 documented skips. Lighthouse completed 60/60 runs across the four configured groups, and LVGL v8/v9 generated fixture compilation passed. Docker validation remains blocked by the local daemon timeout; Arduino AVR/ESP32 compilation remains unavailable because `arduino-cli` is not installed. | Recorded the final CI evidence and environment-only blockers without changing STCC, P4-21, or P4-22 completion status |
| 2026-09-22 | §4.3, §4.7, §26 | Added fresh-page offline and local-only route evidence for T27, T61, T70, T80, and T81. Chromium passed the T27/T70 fresh-page reloads; Firefox/WebKit retain documented local worker/file decode skips. Chromium route measurements recorded 991.8 ms for the 24-file T61 scan, 1006.3 ms for an 8 MP total T80 pair, and 1420.6 ms for a 320-pixel T81 resize. | Narrowed route latency and offline gaps without closing full STCC, broad-device performance, hard-negative quality, or fresh offline startup where it remains unverified |
| 2026-09-22 | §18.3, §25.3.4, §26 | Added a T62 browser-cache proof: after the pinned English model was fetched from jsDelivr and the service worker was warmed, a fresh page recognized with all external requests blocked in Chromium and Firefox without a second CDN request. The true browser-wide offline proof remains limited to bundled Cyrillic because WebKit cannot reload the local File/Blob flow offline. | Narrowed the T62 delivery gap while retaining eviction/private-browsing persistence, expanded-catalogue accuracy, and full STCC as open |
| 2026-09-22 | §13.1.3, §19.2, §26 | Recorded a fresh Chromium production-preview T60 route measurement: a generated 12 MP pair completed in 339.3 ms against the 5 s route budget. The P4-21 report, feature audit, and current audit now distinguish this local route result from broad-device latency, hard comparisons, representative quality, and full STCC. | Closed the missing local T60 route-timing evidence while retaining the remaining quality, error-branch, and STCC gates |
| 2026-09-22 | §13.1.3, §26 | Reconciled the T57 generated fixture suite wording across the README, corpus index, feature audit, and current audit. The suite remains three deterministic project-generated CC0 scenes with five exact boxes and is explicitly limited to functional and edge-case evidence. Corrected the P4-21 T62 row to record bundled-Cyrillic fresh-page offline reuse in Chromium and Firefox after service-worker warm-up; CDN-delivered models remain cache-dependent. | Kept T57 real-photo accuracy, T62 expanded-catalogue accuracy/CDN fresh-start reuse, P4-20 STCC, P4-21 completion, and P4-22 register review open |
| 2026-09-21 | §18.3, §25.3.4, §26 | Added a versioned same-origin service worker for the shell and immutable runtime assets, and registered it from the root layout. Fresh-page offline E2E now passes for T32 Tier 1 in Chromium and for the bundled T62 Cyrillic model in Chromium and Firefox after warm-up; WebKit skips the latter browser-wide offline reload because of its internal local File/Blob limitation. CDN-delivered OCR and Tier 2 model bytes remain browser-cache/IndexedDB dependent and are not claimed fresh-page offline. | Narrowed the offline shortfalls with tested evidence while retaining model-host longevity, cache-eviction, representative quality, and full-STCC limits |
| 2026-09-21 | §4.7, §10.2, §26 | Added T81's strict Zod schema for target width, target height, and protection-mask enablement, with localized generated controls and reset behavior. Zero remains accepted as an intermediate value so the route preserves its typed invalid-dimensions remedy; existing generated-scene measurements and STCC limits are unchanged. | Closed the T81 hand-written dimension/mask-control gap while retaining natural-photo, latency, offline, and full-STCC evidence limits |
| 2026-09-21 | §4.7, §10.2, §26 | Added T80's strict Zod method schema and localized generated segmented control with keyboard operation and reset behavior. Reinhard remains the no-op default; the existing generated texture measurements and route limitations are unchanged. | Closed the T80 hand-written method-control gap while retaining invalid-input branch, latency, offline, quality, and full-STCC evidence limits |
| 2026-09-21 | §4.7, §10.2, §26 | Added T61's strict Zod duplicate-matching schema for average-hash distance, difference-hash distance, and aspect-ratio tolerance, plus localized generated sliders with keyboard operation and reset behavior. Defaults remain 6, 6, and 10%, and changing a control clears prior results. | Closed the T61 generated-control gap while retaining hard-negative, held-out-threshold, route-latency, offline, and full-STCC evidence limits |
| 2026-09-21 | §4.6, §13.1.3, §19.2, §26 | Added the MIT-cleared, revision-pinned YuNet browser adapter and user-started download with an environment-configurable asset URL; selected the deterministic CC0 synthetic face suite with five exact boxes and persisted its manifest, hashes, browser smoke, and limitations. Reconciled the feature audit and current P4-20/P4-21/P4-22 status with the T32/T62 delivery findings. | Kept manual face marking and Tier 1 upscaling available; retained T57 real-photo accuracy, broad T32 quality/hosting, T62 expanded accuracy, P4-20 STCC, P4-21 corpus completion, and P4-22 row review as open |
| 2026-09-21 | §13.1.3, §26 | Added the first individually registered CC0 OCR source: Wikimedia Commons’ Test OCR document, with its API metadata snapshot, source SHA-1/SHA-256, and a declared native-scale English paragraph crop. The production Tesseract.js Chromium 151 measurement returned confidence 94 and one substitution (Alice → Aitice; CER 0.00660066). The result does not establish broad OCR accuracy. | Added the item-level CC0 source and measurement runner/results, updated the T62 corpus and audit summaries, and kept expanded-language accuracy and route STCC open |
| 2026-09-21 | §4.6, §19.2, §13.1.3, §26 | Added route-level T57 E2E coverage for localized static rendering, user-marked and keyboard regions, no-op/export behavior, typed errors, decoder fallbacks, and accessibility/network checks. Chromium, Firefox, and WebKit passed 40/42 checks; the two non-Chromium latency checks were skipped. A generated 6 MP solid-color PNG reached a one-region blur preview in a 229.5 ms median across five Chromium runs (217.8–336.8 ms) against a 3 s route gate. This does not measure detection accuracy or broad-device speed. | Recorded partial route evidence in the T57 benchmark, corpus index, audit, README and plan; detection, labelled accuracy, and full STCC remain open |
| 2026-09-21 | §4.6, §13.1.3, §26 | Selected the deterministic in-repository T57 synthetic face suite released CC0, with three generated PNGs and five exact face boxes, for functional and edge-case measurements only. A fresh Chromium production-preview run matched all five boxes (5 TP, 0 FP, 0 FN; IoU 0.7575–0.8830); first transfer/inference was 2,408 ms and subsequent same-session fixtures took 405 ms and 342 ms. Refreshed the machine-readable browser record and stated explicitly that generated fixtures do not measure real-photo detection accuracy. | Recorded the selected corpus, exact model/fixture hashes, browser measurements, and limits in the T57 report, corpus index, README, and feature audit; real-photo validation and route STCC remain open |
| 2026-09-21 | §11.6, §26 | Added the missing T60 option contract: comparison mode, split position, onion opacity, and difference gain now use a strict Zod schema with generated UI metadata, bounded values, and no-op defaults. The fresh production-preview regression passed 6/6 targeted checks across Chromium, Firefox, and WebKit, including mode switching, reset behavior, keyboard operation, and dimension-mismatch handling. T60's route, comparison quality, latency, and full STCC limitations remain open. | Closed the T60 hand-written-control gap while retaining the existing evidence limits |
| 2026-09-21 | §13.1.3, §26 | Added a reproducible T61 supplement from two item-registered CC0 autumn-leaf photos, with 12 transformed fixtures and 132 directed pairs. The runner applies Chromium's real 8×8 route preprocessing and the current dual-hash thresholds: 34 combined exact+visual true positives, 0 false positives, 26 false negatives, and 72 true negatives (P=1.00, R=0.57; four detections were exact SHA-256 matches). These are topic-matched negatives, not visually near-identical hard negatives; held-out thresholds and route STCC remain open. | Updated the T61 report, corpus index, README summary, plan evidence, and feature audit to reflect measured route-shaped behavior and its limits |
| 2026-09-21 | §4.7, §26 | Corrected the T63 browser E2E flow for WebKit: select the local PNG before toggling the context offline, and inject an unsupported file through a change event so browsers honoring the input accept-filter reach the validation path. The focused three-engine suite passed 22/24 checks; the two skipped cases are Chromium-only clipboard reads. Generated 6 MP preview timing was 175.7 ms on Chromium, 211.6 ms on Firefox, and 656.9 ms on WebKit in this local run. | Fixed browser-specific E2E assumptions and recorded the tested local-only behavior and timings |
| 2026-09-21 | §11.6, §26 | Updated T60's comparison-surface description to match its shipped controls and metric set. The clean production-preview route suite passed 27/27 checks across Chromium, Firefox, and WebKit, including typed unsupported/oversize/decode/worker errors, dimension-mismatch handling, worker cancellation and termination, and keyboard navigation/activation. The limited proxy score, untested image-too-large/canvas-unavailable branches, route latency, hard comparisons, representative quality, and full STCC remain open. | Reconciled the T60 product description, route evidence, and audit without claiming unmeasured latency or representative quality |
| 2026-09-21 | §4.7, §19.2, §26 | Added T63's manual `/alt-text` authoring route with bounded local preview, a 125-character draft, decorative-image and human-review controls, and an escaped copyable HTML attribute preview. The focused production-preview Chromium suite passed 8/8; a generated 6 MP selection-to-preview measured 149.3 ms in the latest local run. The root production build and all route-size and application-size gates passed; raw-converter measured 116,599/117,000 compressed JS bytes after the Arabic strings were split into a T63-only chunk. The route remains manual-only and its full STCC is open. Corrected T60/T80/T81 audit notes to match checked-in route E2E coverage and marked their verified tests accurately. | Recorded route and measured preview evidence without claiming automated accessibility analysis, broad-device performance, or full STCC completion |
| 2026-09-21 | §4.10, §13.1.3, §26 | Extended T80 route E2E with simulated decode failure, worker-construction failure, and explicit cancellation/worker-termination checks. The focused isolated production-preview suite passed 27/27 across Chromium, Firefox, and WebKit. These tests improve error-path evidence but do not add browser-scale latency or visual-quality evidence; invalid-PNG, canvas-unavailable, and missing-image errors, offline operation, representative image preference, and full STCC remain open. | Reconciled T80's README, feature audit, benchmark report, and P4-20 plan notes to the verified route suite while keeping the no-model-gap and quality limits explicit |
| 2026-09-21 | §4.7, §19.2, §26 | Re-measured the production route bundles after adding the generated T60/T61/T80/T81 controls. The RAW converter measured 117,131 compressed JavaScript bytes, so its dedicated guard now allows 118,000 bytes while the shared tool-route ceiling remains 115,000. | Kept the route-size regression guard aligned with the shipped generated-control bundle without changing any route behavior |
| 2026-09-21 | §16, §26 | Re-ran the CI component matrix after the generated-control work: the full workspace test (1,052 engine tests), gates, build, lint, typecheck, no-network E2E (6/6), Lighthouse (60/60), RAW corpus, credential-leak check, and LVGL v8/v9 C11 compiles passed. The full browser matrix completed with 1,086 passed and 32 skipped; a Firefox T63 hydration race was stabilized and its focused retry passed 3/3. Docker validation remains host-blocked because the local daemon does not respond, and AVR/ESP32 compilation could not run because `arduino-cli` is not installed on this machine. | Recorded the current verification evidence and environment-only limits without changing P4-20 STCC, P4-21, or P4-22 completion status |
| 2026-09-22 | §16, §26 | Re-ran the full CI-mode three-browser Playwright matrix after adding the T62 fresh-page CDN-cache proof: 1,088 passed and 39 documented skips. One existing WebKit JPEG XL case was accepted after retry; no new failure remained. The focused T62 English CDN cache proof passed in Chromium and Firefox, while WebKit retains the documented browser-wide offline limitation. Docker validation remains host-blocked because the local daemon does not respond, and AVR/ESP32 compilation could not run because `arduino-cli` is not installed on this machine. | Recorded the expanded browser-matrix evidence and current environment limits without changing P4-20 STCC, P4-21, or P4-22 completion status |
| 2026-09-21 | §4.8, §26 | Added T61 route checks for per-file and total byte limits, unsupported MIME, the 24 MP ceiling, and Web Crypto failure. The focused isolated production-preview suite passed 27/27 across Chromium, Firefox, and WebKit, now exercising each declared route error kind and remedy. Visual hard negatives, held-out thresholding, Zod-generated controls, complete SEO/i18n evidence, route latency, offline operation, and full STCC remain open. | Reconciled the T61 route suite, feature audit, README, P4-21 report, and P4-20 notes without extending its quality claims |
| 2026-09-20 | §16 | Re-ran the local CI component matrix after fixing the Smart Crop package export: lint, typecheck, unit tests and gates, build, no-network, credential-leak, all four Lighthouse batches (60 fresh runs), the 31-file RAW corpus, LVGL v8/v9 plus AVR/ESP32 embedded compiles, full browser E2E (923 passed, 9 skipped; one JXL retry), and Docker validation all passed. Capped CI Playwright workers at two after six-worker contention interrupted Firefox navigation; extended the multi-encode JXL case timeout after it passed on retry and in isolation. The first local Docker run included a temporary `.lvgl` checkout; excluded that test-only path from Git and Docker contexts, then Docker validation passed with a 370,762,587-byte image. | Recorded post-fix component evidence; kept browser-dependent variability visible and excluded only local test checkout data from packaging |
| 2026-09-20 | §4, §13.1.3, §16, §26 | Added the explicit `@complianttools/image-engine/ops/smart-crop` package export that CI's Svelte/Vite build could not resolve. The focused web lint and production build pass. Added a reproducible 12-case T27 crop-placement report from four item-registered CC0 images, documenting its subjective-box limits; refreshed T79's 15-case report and register from the latest seven-repeat run; corrected T79's feature maturity and added the T32 hosting assessment with the pinned model identities, current third-party host caveat, runtime URL overrides, and remaining route E2E gaps. | Reconciled P4-20 package-route resolution and P4-21/P4-22 T27/T32/T79 evidence while retaining full route STCC and model-host durability gates |
| 2026-09-20 | §4, §13.1.3, §26 | Added T32's local DCCI/NEDI worker route with still-PNG validation, bounded preview/export, cancellation, source replacement, localized discovery pages, and a sitemap entry. The fresh production-preview Chromium route suite passed 9/9 and the separate active-source-replacement regression passed 1/1. No Tier 2 model request is needed; output-byte-boundary coverage, Tier 2 hosting, real-camera comparison, and full STCC remain open. | Updated P4-20 route evidence and feature audit while retaining Tier 2, representative-quality, and full STCC gates |
| 2026-09-20 | §4.7 | Added a visible T60 cancellation control that discards results from pending decode work, terminates active metric workers, and reports a typed cancellation with a restart remedy. The focused Chromium production-preview E2E passed 8/8, including explicit worker-termination coverage; its limited proxy metrics and remaining STCC gaps remain open. | Updated the P4-20 evidence and feature audit while retaining T60's route-latency, representative-quality, and full STCC work |
| 2026-09-20 | §4, §13.1.3, §25.3.2 | Added T44's reproducible median/bilateral comparison on 12 deterministic synthetic-noise cases derived from four item-registered CC0 crops, with exact hashes, PSNR/SSIM, latency, and explicit scope limits. Corrected the AI register, corpus index, feature audit, denoising catalog, and IP clearance/algorithm tables to state that wavelet/BayesShrink is not implemented; non-local means remains excluded. | Added T44 measured evidence to P4-21 and reconciled its P4-22 register status while retaining route STCC and real sensor-noise evidence as open |
| 2026-09-20 | §4.8, §7.3, §13.1.3 | Expanded the T60 route E2E to 7/7 focused Chromium cases: CC0/JPEG measurement, comparison modes, dimension-mismatch suppression, typed unsupported/oversized/decode/worker errors, keyboard-only controls, Axe, and no off-origin requests. Its ≤256 px metric proxy and remaining STCC limits remain open. Added a repeatable T66 CC0 photo-derived synthetic-occlusion proxy: four registered source photos, eight 32×32 cases, five methods, 40 hashed case outputs and 64 PNG artifacts; the repeated run reproduced output hashes. Telea led masked-region mean PSNR/SSIM (25.3583 dB / 0.610996); Navier–Stokes had the fastest median (0.320 ms). Updated claims to state this is not ground truth for actual object removal. A production Docker build completed after fixing the patch copy layer and excluding generated host builds/model caches from its context; inspection found only the 29,252,466-byte Cyrillic `.traineddata` file in the final nginx image. Other selected OCR models are fetched directly by the browser from pinned CDN sources and reused in browser storage where available. Recorded the exact T32 x2/x4 ONNX sizes and remaining immutable-host, consent, integrity-check, persistent-cache, and offline-reuse gates. Corrected the T32 benchmark degradation description to blur/grain/JPEG and clarified that the existing consent flag gates the engine adapter, not a product consent UI; also corrected the T44 register to distinguish implemented median/bilateral methods from the unimplemented wavelet candidate. | Reconciled P4-21 and P4-22 T60/T66/T62 evidence and the T60 feature row; kept STCC, broad quality, and T32 product delivery open |
| 2026-09-20 | §13.1.3, §16, §26 | Added measured T70/T80/T81 synthetic comparisons and reproducible artifacts. T70 exposed invented palette colors and records the strict-majority fix plus transparent-alpha tradeoff; T80 validates distribution matching across different image sizes and records its metric limits; T81 exposed inverted protect-mask weighting, then was corrected to reserve protected rows/columns exactly, with before/after artifacts and the remaining 8.08 px position shift recorded. | Updated feature claims and P4-21/P4-22 evidence without closing route STCC or general-quality gaps |
| 2026-09-20 | §7.3, §13.1.3, §16.4, §25.3.4, §26 | The metadata-only OCR catalogue audit found a second jsDelivr 403: the 89,384,811-byte Latin script model. Added an exact pinned raw GitHub fallback used only when Latin is selected; the browser worker returned OCR output from that URL, and a streamed source check verified the registered size and SHA-256. The Latin option shows its 85.2 MiB size before OCR runs. The named cache preparer also downloaded and verified Latin, then the temporary ignored file was removed so only Cyrillic remains bundled. Added `pnpm ocr:verify-catalog`; it checks 161 other jsDelivr rows by status, identity content length, pinned revision, CORS and CORP without downloading the catalogue. | Resolved the oversized T62 script-model delivery gap without bundling another model; kept its accuracy unmeasured and all default helper/script loading lazy |
| 2026-09-20 | §7.3, §20.1, §26 | Localized OCR catalogue labels through standard browser language/script names, including Tesseract language variants, vertical/legacy entries, scripts, and the deprecated alias. Browser assertions cover all 124 Arabic language labels and all 37 Arabic script labels; pseudo-locale assertions cover every language and script option. The rebuilt production-preview Chromium OCR suite passed 12/12 with only Cyrillic in the static tessdata cache, including keyboard-only selection/recognition/download and same-page offline repeat of all eight warmed language fixtures with identical outputs and zero external requests. | Verified current T62 browser suite and same-page model-cache reuse; fresh-page offline startup, expanded-language accuracy, and remaining STCC criteria stay open |
| 2026-09-20 | §13.1.3, §13.1.4, §26 | Added an executable T60/T61 corpus from 24 source-linked variants of four registered CC0 images. T60 recorded 26 metric comparisons, with exact-copy and JPEG quality ordering checks passing 4/4; T61 recorded 552 directed retrieval pairs and fixed-threshold P/R of 1.00/0.55 for average hash and 1.00/0.20 for difference hash. Cross-source negatives are easy and misses remain high; neither result justifies a broad product claim. | Replaced placeholder T60/T61 benchmark stubs with reports/results and added limited P4-21 evidence; route STCC and harder comparison/retrieval corpora remain open |
| 2026-09-20 | §7.3, §20.1, §26 | Added a keyboard-only T62 E2E for selecting an image through the file picker, running English OCR, and downloading the exact displayed text. The focused case passed 1/1 on Chromium against the current preview while English was absent from the local cache; Axe reported zero violations. The earlier full OCR suite remains 9/9 from before this new case and has not yet been rerun at full-suite scope. | Added verified keyboard-path evidence for T62 STCC item 10; retain T62 and P4-20 as partial/open while the other STCC criteria remain unproven |
| 2026-09-20 | §13.1.3, §26 | Recorded the T32 controlled synthetic ×4 comparison: Real-ESRGAN loses to Tier 1 on each tested case; evaluation-only Swin2SR q4f16 modestly improves aggregate metrics but has mixed class results, slow inference, and unresolved per-file license/training-data provenance. Updated the corpus index date and persisted OCR fixture links; distinguished T79's local-outcome benchmark gap from its no-model-gap status and kept T80 Tier 3 unproven on the current synthetic metric; corrected stale change-log section references and made P4-22 evidence acceptance status-specific. | Kept T32 selection and P4-21/P4-22 open; no escalation removed, and blocked/no-model-gap/AI-only rows retain evidence appropriate to their disposition |
| 2026-09-20 | §7.3, §13.1.3, §13.1.4, §26 | Added a Playwright opt-out so focused OCR E2E runs through the standard preview with the one-model cache; Chromium passed bundled Cyrillic recognition and mapped a forced pinned-CDN 503 to the download-specific remedy. The full OCR Chromium suite passed 9/9 with normal English/OSD prefetch; the ignored cache was pruned back to Cyrillic. Persisted eight self-generated 1200×160 OCR PNG fixtures (115,246 bytes total) with PNG/RGBA hashes; the saved pixels exactly match the earlier benchmark inputs. Reran the benchmark against those files with the same 7/8 exact, 0.00735 mean CER. An uncached Arabic model also loaded through the pinned CDN browser path and returned an OCR result. The 11 language/helper files needed for the benchmark were pruned afterward, leaving only Cyrillic. | Strengthened P4-18 error/test coverage and P4-21 OCR reproducibility/CDN delivery evidence; expanded-catalogue accuracy, full offline guarantees, and P4-20 STCC remain open |
| 2026-09-19 | §7.3, §13.1.3, §13.1.4, §25.3.4, §26 | Registered all 163 recursive official `tessdata_fast` tree entries at the pinned upstream revision (123 language/variant model binaries, one deprecated `frk` alias pointer, 37 script model binaries, and the `equ`/`osd` helper binaries; 700,353,193 bytes including the pointer). Added a standalone `/ocr` route with 124 language/variant choices, 37 script models, OSD orientation/script results, equation transcription, localized prerendered discovery content, and recognition-triggered Tesseract loading; installed Edge passed all six OCR page checks, including a 270° correction for a generated image rotated 90° clockwise, and same-origin language OCR/download. Model delivery now prefers a present local cache, then the exact pinned jsDelivr URL; only the 29,252,466-byte Cyrillic model remains bundled because its CDN request returns 403. Explicit test/benchmark preparers populate ignored cache files only when needed and verify size/SHA-256; clean production builds prune the cache. The OCR page recommends upright images and keeps OSD and Script models unloaded unless selected. The earlier measurements are unchanged. Same-origin Chromium OCR accuracy evidence still covers only the original eight generated-print fixtures (7/8 exact, mean CER 0.00735); Hausa is absent. Recorded the project-owner BSD-3 label decision and exact hashes for official Real-ESRGAN x2/x4 checkpoints, converted both to dynamic ONNX, passed CPU parity, and ran x2/x4 through browser WASM without external requests. Added four CC0 image sources, T32 Tier 1 + x2 measurements, and a generated T66 comparison of five methods on three known-target textures (21 hashed artifacts). T32 x2 scored 26.6370 dB / 0.765509 mean SSIM versus DCCI 29.6861 / 0.827896 on clean synthetic 2× pairs; T66 remains a narrow synthetic result. Corrected unsupported or uncleared T27/T67/T68/T69/Denoise descriptions and reconciled the corpus index. Full offline shell caching, representative photo comparisons, and other register entries remain open. Pinned `onnxruntime-web@1.30.0` with MIT licence-gate evidence. | Kept P4-14/16/18/21/22 at verified partial milestones; engine-level P4-20 integration is recorded complete while P4-20 STCC and the Phase 4 gate remain open |
