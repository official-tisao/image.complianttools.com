...
- [/] Lazy engine API, WebGPU/WASM selection, consent gating, and Tier 1 fallback are wired; production download size disclosure, persistent model caching, and offline re-use still need browser evidence
...
- [x] Reproducible dynamic-spatial ONNX conversion and CPU parity passed for both checkpoints; the actual x2 model also ran in browser WASM
...
- [/] Representative degraded-photo and x4 comparisons plus product model hosting/download flow remain open; do not infer a general restoration benefit from the synthetic interpolation result
...
- [/] Engine primitives or adapters exist for most listed tools. T62 and T70 now have dedicated product routes. T70's 16-case route suite passes in Chromium and installed Edge; Firefox and WebKit each pass all 15 functional cases and skip only the Chromium/Edge-only latency measurement. It covers each typed error kind and remedy, byte-identical no-op/export parity, keyboard flow, same-page offline reuse, and the 1 MP ×2 budget. The 320 CSS-pixel layout check passes without horizontal overflow in all four browsers. Actual 400% browser zoom remains unverified, so T70 STCC remains partial. T32 has a consent-checking ONNX adapter only: it requires caller-supplied converted weights and has no route-level size disclosure, consent UI, downloader, or persistent-cache flow. T61 reuses shared analysis primitives; T63 has no dedicated accessibility pipeline. All eleven STCC rows remain open.
...
- [/] README §13.1.3 assigns Real-ESRGAN to T32 (T70 remains pixel-art-only), records the evaluation-only Swin2SR x4 comparison without selecting it for product use, reflects the 163-entry T62 catalogue, the pinned raw-source fallback for the oversized Latin script file, and persisted eight-fixture OCR measurement without broadening accuracy claims, records T32 clean x2 and controlled synthetic x4 plus T66 evidence, and corrects unsupported/uncleared T27/T67/T68/T69/Denoise claims. P4-21 now includes initial T60/T61 results, measured T70 palette behavior, T80 distribution matching, and T81 subject geometry; remaining entries retain their evidence-based partial, blocked, or no-model-gap status, with route STCC still open.
