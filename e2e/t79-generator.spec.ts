...
for (const locale of ['en', 'en-XA', 'ar'] as const) {
  test(
    `T79 ${locale} route has localized prerendered page metadata and controls`,
    async ({ page }) => {
...
test('T79 seeded noise preview and downloaded PNG use stable local bytes', async ({ page }) => {
  ...
  await expect(page.getByTestId('t79-status')).toContainText('Created a 64 × 48 pixel PNG locally.');
  ...
  const repeatedBytes = Buffer.from(
    await page.evaluate(
      async (url) => Array.from(new Uint8Array(await (await fetch(url!).arrayBuffer()))),
      repeatedUrl,
    ),
  ...
  const valueNoiseBytes = Buffer.from(
    await page.evaluate(
      async (url) => Array.from(new Uint8Array(await (await fetch(url!).arrayBuffer()))),
      valueNoiseUrl,
    ),
...
test(
  'T79 radial gradient runs and oversized dimensions receive an accessible bound error',
  async ({ page }) => {
