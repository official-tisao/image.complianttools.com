...
for (const locale of ['en', 'en-XA', 'ar'] as const) {
  test(`${locale}/smart-crop is prerendered with localized SEO`, async ({ page }) => {
    ...
    await expect(page.locator('link[rel=canonical]')).toHaveAttribute('href', `https://image.complianttools.com${prefix}/smart-crop`);
    await expect(page.locator('meta[name="description"]')).toHaveAttribute('content', /.+/u);
    await expect(page.locator('script[type="application/ld+json"]')).toHaveCount(1);
  });
}

test('T27 uses local center, thirds, and approximate saliency crops and the PNG download matches its preview', async ({ page }) => {
  ...
  await page.getByTestId('t27-file-input').setInputFiles({ name: 'pattern.png', mimeType: 'image/png', buffer: await generatedPng(page) });
  ...
  for (const method of ['thirds', 'saliency'] as const) {
    await page.getByRole('radio', { name: new RegExp(method === 'thirds' ? 'Rule-of-thirds' : 'Visual-saliency', 'u') }).check();
    await page.getByTestId('t27-run').click();
  ...
  const previewBytes = await page.evaluate(async (url) => Array.from(new Uint8Array(await (await fetch(url!)).arrayBuffer())), previewUrl);
...
test('T27 rejects unsupported and dimension-bomb inputs before processing', async ({ page }) => {
  ...
  await expect(page.getByTestId('t27-error')).toHaveAttribute('data-error-kind', 'unsupported-file');

  const tooLarge = await generatedPng(page);
  tooLarge.writeUInt32BE(4_000, 16);
  tooLarge.writeUInt32BE(4_000, 20);
  await input.setInputFiles({ name: 'dimensions-too-large.png', mimeType: 'image/png', buffer: tooLarge });
